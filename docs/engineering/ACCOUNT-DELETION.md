...
La recette PostgreSQL locale applique les 72 clés étrangères capturées sur le schéma existant. Elle teste les reprises, les droits, la conservation d’un second participant, les fichiers et la fin de vie d’Auth simulés. Les tests HTTP exécutent le vrai gestionnaire avec des services Auth/Storage simulés. Ils ne constituent pas un test Supabase réel ; les tests navigateur du nouveau parcours restent à effectuer, l’accès à la recette locale ayant été refusé lorsque le contrôle de politique du navigateur était indisponible.
