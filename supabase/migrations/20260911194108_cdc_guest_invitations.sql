...
begin
 ...
 if exists(select 1 from jsonb_each_text(requested_availability) v where v.value is null or v.value not in ('yes','maybe','no') or not exists(select 1 from public.moment_date_options o where o.id::text=v.key and o.moment_id=parent.id)) then return jsonb_build_object('error','invalid_response'); end if;
