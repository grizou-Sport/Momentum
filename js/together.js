    ...
    <article class="moment-card" style="--moment-color:${colors[moment.moment_type] || colors.OTHER}">
      ...
      <div class="moment-card-details">
        ...
        <div class="moment-card-footer">
          <span>${participantCount} membre${participantCount > 1 ? "s" : ""} MOMENTUM</span>
...
async function openClubDetail(clubId) {
  ...
  const { data: passports } = userIds.length ? await window.momentumDB.rpc("shared_profiles",{p_user_ids:userIds}) : { data: [] };
...
async function openMomentDetail(momentId) {
  ...
  if (!moment) return;
  elements.momentDetail.innerHTML = '<div class="detail-loading"><h2 id="momentDetailTitle">Ouverture du Moment…</h2></div>';
  elements.momentDetailDialog.setAttribute('aria-labelledby', 'momentDetailTitle');
  ...
  const [mediaResult, linksResult, activitiesResult, reactionsResult, presetsResult, participantsResult] = await Promise.all([
    ...
    window.momentumDB.rpc("shared_moment_activities", {p_moment_id:moment.id}),
  ...
  if (loadError) {
    elements.momentDetail.innerHTML = '<div class="dialog-shell"><button class="dialog-close detail-close" type="button" aria-label="Fermer le Moment">×</button><h2 id="momentDetailTitle">Moment indisponible</h2><p>Le chargement a échoué. Ferme cette fenêtre puis réessaie.</p></div>';
  ...
  elements.momentDetail.innerHTML = `<div class="dialog-shell moment-memory-shell"><button class="dialog-close detail-close" type="button" aria-label="Fermer le Moment">×</button><span class="card-label">${escapeHTML(moment.status)}</span><h2 id="momentDetailTitle">${escapeHTML(moment.title || "Moment à préparer")}</h2><p class="detail-lead">${escapeHTML(moment.description || "Un Moment partagé.")}</p><div class="detail-facts"><span>${escapeHTML(formatDate(moment.start_at))}</span><span>${escapeHTML(moment.location_name || "Lieu à définir")}</span><span>${escapeHTML(visibilityLabel(moment.club_id ? "Club" : moment.visibility))}</span></div>
    ...
    <section class="detail-section"><div class="detail-title"><h3>Membres MOMENTUM</h3><span class="section-count">${participants.length}</span></div><div class="moment-participant-list">${participants.length ? participants.map((participant) => { const profile = participantProfileMap.get(participant.user_id) || {}; const name = participant.user_id === TOGETHER.user.id ? "Toi" : profile.display_name || "Membre MOMENTUM"; return `<div class="moment-participant-row">${circleIdentity(profile, "mini-avatar")}<span><strong>${escapeHTML(name)}</strong><small>${escapeHTML(participant.role === "OWNER" ? "Organisateur" : "Participant")}</small></span><span class="participant-invitation-status status-${participant.invitation_status.toLowerCase()}">${escapeHTML(participant.participation_status === 'WAITLISTED' ? 'En attente d’une place' : participant.invitation_status === 'ACCEPTED' && participant.confirmed_schedule_revision !== moment.schedule_revision ? 'À reconfirmer' : invitationStatusLabel(participant.invitation_status))}</span></div>`; }).join("") : '<p class="memory-empty">Aucun participant invité.</p>'}</div></section>
    ...
    <section class="detail-section"><div class="detail-title"><h3>Activités liées</h3>${availableActivities.length ? `<select id="linkActivity" aria-label="Lier une activité HOME"><option value="">Lier une activité HOME…</option>${availableActivities.map((activity) => `<option value="${activity.id}">${escapeHTML(sportLabel(activity.sport) || activity.activity_type || "Activité")} · ${escapeHTML(activity.activity_date || "Sans date")}</option>`).join("")}</select>` : ""}</div><div class="linked-activities">${(linksResult.data || []).length ? (linksResult.data || []).map((link) => activityMemoryCard(link.activities)).join("") : '<p class="memory-empty">Aucune activité liée.</p>'}</div></section>
  ...
  document.getElementById("guestInvitations")?.addEventListener("click",()=>window.MomentumGuests.open(moment));
...
function setTogetherView(view) {
  ...
  if (elements.primaryAction) elements.primaryAction.textContent = view === "moments" ? "Proposer un Moment" : "Inviter dans mon Cercle";
...
function openTogetherSection(section, updateHistory = true) {
  ...
  const next = valid.includes(section) ? section : "moments";
  ...
  if (elements.primaryAction) {
    ...
    elements.primaryAction.textContent = next === "moments" ? "Proposer un Moment" : next === "clubs" ? "Créer un Club" : "Inviter dans mon Cercle";
  }
  if (updateHistory) window.history.pushState({}, "", `together.html?view=${next}`);
...
renderSportOptions();
function requestedTogetherSection() {
  const params = new URLSearchParams(window.location.search);
  return params.get("section") || params.get("view") || window.location.hash.slice(1) || "moments";
}
window.addEventListener("popstate", () => openTogetherSection(requestedTogetherSection(), false));
loadTogether().then(() => openTogetherSection(requestedTogetherSection(), false));
