            ...
            ${activeSports
              .map((sport) => `
                <div class="you-small-card sport-practice-card${sport.level === "regular" ? " active" : ""}" data-practice-level="${sport.level}">
                  ...
                  <em>${escapeHTML(sport.levelLabel)} · ${sport.sessionCount ? `${sport.sessionCount} séance${sport.sessionCount > 1 ? "s" : ""} sur 12 mois` : sport.questionnaireRole === "Principal" ? "terrain principal du questionnaire" : "choisi lors du questionnaire"}</em>
                </div>
              `)
              .join("")}
          </div>
