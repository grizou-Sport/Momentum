...
(function(){
 ...
 async function open(moment){
  ...
  document.body.append(dialog);const form=dialog.querySelector('form'),message=dialog.querySelector('[data-message]'),list=dialog.querySelector('[data-list]');let creation=null,loadErrorMessage=null;
  ...
  async function load(){
   ...
   try{const result=await request('list_guest_invitations',{p_moment_id:moment.id});if(!dialog.isConnected)return;
    if(loadErrorMessage===message.textContent)message.textContent='';loadErrorMessage=null;
   ...
   }catch(error){list.textContent='Les réponses ne sont pas disponibles. Utilise Actualiser.';report(error);loadErrorMessage=message.textContent;}
  }
